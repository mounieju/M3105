...
public class Move extends Action
{
	...
	public static void simpleMove(Map map, Character pers, CellPosition target)
	{
		...
		{
			moveCharacterOnMap(map, pers, target, deltaX, deltaY);
		}
	}

	private static void moveCharacterOnMap(Map map, Character pers,CellPosition target, int deltaX, int deltaY) 
	{
	...
	public static ArrayList<CellPosition> getAccessiblePos(Map map, Character pers)
	{
		...
		for (int y = pers.getCellTraveled().getPosition().getPositionY() - map.getTeamController().getPlayingTeam().getActionPointsLeft() ;
			 ...
			 y++)
		 {
			getEnableMove(map, pers, cellList, y);		
		 }
		
		return cellList;
	}

	private static void getEnableMove(Map map, Character pers,ArrayList<CellPosition> cellList, int y)
	{
